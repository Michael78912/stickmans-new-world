...
USR_NAME = '.stickman_new_world{0}user_settings.json'.format('\\' if os.name == 'nt' else '/')
HOME = 'C:\\Users\\{}\\'.format(os.getlogin()) if os.name == 'nt' else '/home/{}/'.format(os.getlogin())
print(HOME)
print()

SETTINGS_FILE_PATH = {
	'posix': HOME + USR_NAME if os.path.exists(HOME + USR_NAME) else 'settings.json',
	'nt': HOME + USR_NAME if os.path.exists(HOME + USR_NAME) else 'settings.json'
}[os.name]

SETTINGS_FILE = open(SETTINGS_FILE_PATH).read()

USR_SAVE_PATH = HOME + '{0}.stickman_new_world{0}'.format('\\' if os.name == 'nt' else '/')
...
def main():
	...
	for key, value, num in zip(settings, settings.values(), range(len(settings))):
		print(key, value, usr_settings)
		tk.Checkbutton(root, text=key, variable=usr_settings[key]).grid(row=num, sticky=tk.W)
