...
class Blob(Enemy):
	...
	def move(self, all_players, surface):
		...
		possible_destinations = [abs(self.pos[1] - destination) for destination in possible_destinations]
		destination = min(possible_destinations)
		destination_player = all_players[possible_destinations.index(destination)]
