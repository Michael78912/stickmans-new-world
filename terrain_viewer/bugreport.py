...
EMAILFILE = os.path.join((os.environ['USERPROFILE'] if os.name == 'nt' else os.environ['HOME']), '.stickman_new_world', 'useremail')
...
def send_message(message):
    ...
    try:
        ...
        server.sendmail('bugreporter.smr@mail.com', ['bugreporter.smr@mail.com'], message)
    except Exception as e:
        print(e)
        threading.Thread(target=lambda: error('Another Error', 'an error accured when trying to send the email.\n please contact me at <michaelveenstra12@gmail.com>'))
    server.close()
